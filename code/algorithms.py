...
"""
This is the core programm. It contains the weight function and the A* implementation.
"""
class Node:
...
class AStar:
    ...
    def getg(self,node1,node2,currentdistance = None,bias = True): # set bias = False for time calculation of finished route
        if currentdistance == None: # true for non-grid data
            ...
            if delta_z >= 0: # for going uphill
                ...
                if slope > 1: k = 5 # default slope > 1: k = 5 
                else: k = 1
                tv = delta_z * 9 # equals delta_z [m] * 3600s/h / 400m/h, time uphill vertical move default 9.0
                if bias: 
    ...
    def visualize(self,path, fulldata = None, showsearched = False):
        #plt.close('all')
        ...
        pathlist = []
        if type(path) is not None: # when plotting path
            listpath = list(path)
...
def main(directory,griddistance,startpos,endpos):
    ...
    astar.visualize(path=path,fulldata=data)
